...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
   (let ([zero (make-wire)])
     (set-signal! zero 0)
     (define (foo a b s carry)
       (if (null? a) carry
           (let ([c-out (make-wire)])
             (full-adder (car a) (car b) carry (car s) c-out)
             (foo (cdr a) (cdr b) (cdr s) c-out))))
    (let ([CN (foo wire-lstA wire-lstB wire-lstS zero)])
      (add-action! CN (lambda () (set-signal! wireCN (get-signal CN)))))))
