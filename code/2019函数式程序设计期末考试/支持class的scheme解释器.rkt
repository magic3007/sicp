...
(define (analyze exp)
  (match exp
    [`(class ,x . ,args) (analyze-class x args)]
    [`(new ,x) (analyze-new x)]
    [`(set! (,x ,var) ,e) (analyze-class-set! x var e)]
    [_
   (cond 
...
(define (analyze-class name args)
  (define (helper arg-text)
    (match arg-text
      [`(define ,var ,e) `(,var ,(analyze e))]))
  (let* ([pairs (map helper args)])
    (lambda (env)
      (define-variable! name pairs env)))
)

(define (analyze-new name)
 (lambda (env)
  (let* ([pairs (lookup-variable-value name env)]
         [vars (map first pairs)]
         [vals (map (lambda (x) ((second x) env)) pairs)])
    `(instance ,(extend-environment (list->mlist vars) (list->mlist vals) env)))))

(define (analyze-class-set! inst var e)
  (let* ([finst (analyze inst)]
         [fe (analyze e)])
    (lambda (env)
      (match (finst env)
        [`(instance ,inst-env)
          (set-variable-value! var (fe env) inst-env)
        ]
      ))
    ))
  ...
  (let ((fproc (analyze (operator exp))) 
     ...
     (lambda (env)
      (match (fproc env)
        [`(instance ,env)
          (match exp
            [`(,x ,var) (lookup-variable-value var env)])
         ]
      [_ (execute-application (fproc env)
