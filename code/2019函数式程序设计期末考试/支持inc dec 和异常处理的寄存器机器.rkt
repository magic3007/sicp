...
(define (make-machine register-names ops controller-text exception-text)
  (let ((machine (make-new-machine)))
    ...
    ((machine 'install-exception-sequence)
        (assemble exception-text machine))
    machine))
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'install-exception-sequence)
                (lambda (seq) (set! the-exception-sequence seq))
                )
              ((eq? message 'trap)
               (set-contents! pc the-exception-sequence))
...
(define (make-inc inst machine pc)
    (match inst
        [`(inc ,reg-name)
        (lambda ()
            (if (number? (get-register-contents machine reg-name))
                (begin
                    (set-register-contents! machine reg-name 
                        (add1 (get-register-contents machine reg-name)))
                    (advance-pc pc))
                (machine 'trap)
                ))]
    [_ (error "make-inc")]))
    ...
    (match inst
        ...
        (lambda ()
            (if (number? (get-register-contents machine reg-name))
                (begin
                    (set-register-contents! machine reg-name 
                        (sub1 (get-register-contents machine reg-name)))
                    (advance-pc pc))
                (machine 'trap)))]
    [_ (error "make-dec")]))
...
(define (make-machine-from-text machine-text)
  ...
  (define exception (cadr (cadr (cdddr machine-text))))
