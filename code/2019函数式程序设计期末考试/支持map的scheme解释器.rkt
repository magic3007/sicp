...
(define (analyze exp)
  (match exp
    [`(map ,proc . ,lsts) (analyze-map proc lsts)]
    [_ 
   (cond 
...
(define (analyze-map proc lsts)
  (let* ([fproc (analyze proc)]
         [flsts (map analyze lsts)])
    (lambda (env)
      (let* ([args (map (lambda (flst) (flst env)) flsts)]
             [reorder (apply map (lambda x x) args)]
             [tf (fproc env)])
        (map (lambda (arg) (execute-application tf arg))
             reorder)))))
