...
(define (run-fproc fproc args env)
  (match ((force-fexp fproc) env)
    [(ProcedureType syms fbody proc-env)
     (fbody (EnvType (make-frame-from-pairs (map (lambda (x y) `(,x ,(delay-it y env)))
                                                   syms args)) proc-env))]
    [primitive-proc #:when(eq? primitive-proc cons)
                    (let* ([vals (map (curryr delay-it env) args)])
