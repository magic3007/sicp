; http://lisp.test.openjudge.org/2020hw132/002/
...
(define (make-swap inst machine pc)
  (match inst
    [`(swap ,a ,b)
     (let ([ra (get-register machine a)]
           [rb (get-register machine b)])
       (lambda ()
         (define temp (get-contents ra))
         (set-contents! ra (get-contents rb))
         (set-contents! rb temp)
         (advance-pc pc)))]
    [_ (error "SWAP instruction format error: " inst)]))
      ...
      (let ((m-txt (read)))
            (if (eq? m-txt eof)
                ...
                (if (eq? (car m-txt) 'make-machine)
                    (begin 
                        (set! machine (let/cc k (set! glb-cc k) (make-machine-from-text m-txt)))
                        (displayln "a new machine")
                        (when (eq? machine 'label-error) (displayln "label error in machine"))
                        (if (eq? machine 'label-error)
                          (begin (read) (read))
